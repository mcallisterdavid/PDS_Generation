...
save_dir = 'results/%s_reverse_process_loss_exact/%s_lr%.3f_seed%d_noiseshare' % (args.model, args.prompt.replace(' ', '_'), args.lr, args.seed)
...
for step in tqdm(range(args.n_steps)):
    ...
    noise_pred = pds_dict['noise_pred']
    noise_pred_prev = pds_dict['noise_pred_prev']
    latents_noisy_prev = pds_dict['latents_noisy_prev']
    if step == 0:
        im = pds_dict['x_0_hat_pred']

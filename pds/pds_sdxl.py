...
@dataclass
class PDS_sdxlConfig:
    sd_pretrained_model_or_path: str = "stabilityai/stable-diffusion-xl-base-1.0"
...
class PDS_sdxl(object):
    def __init__(self, config: PDS_sdxlConfig):
        self.config = config
        self.device = torch.device(config.device)

        self.pipe = StableDiffusionXLPipeline.from_pretrained(config.sd_pretrained_model_or_path).to(self.device)
        ...
        self.tokenizer2 = self.pipe.tokenizer_2
        self.text_encoder2 = self.pipe.text_encoder_2
        ...
        self.null_text_feature, self.null_pooled_text_feature = self.encode_text("")
        ...
        add_time_ids = self._get_add_time_ids(
            original_size, crops_coords_top_left, target_size, dtype=torch.float32
        )
    ...
    def encode_image(self, img_tensor: Float[torch.Tensor, "B C H W"]):
        ...
        imgs = imgs * 2.0 - 1.0
        posterior = self.vae.to(dtype=input_dtype).encode(imgs).latent_dist
        latents = posterior.sample() * self.vae.config.scaling_factor
        return latents

    def encode_text(self, prompt):
        prompt_embeds_list = []
        for tokenizer, text_encoder in zip([self.tokenizer, self.tokenizer2], [self.text_encoder, self.text_encoder2]):
            with torch.no_grad():
                tokens = tokenizer(
                    prompt,
                    padding="max_length",
    ...
    def decode_latent(self, latent):
        ...
        image = self.vae.decode(latent.to(dtype=torch.float32)).sample
    ...
    def update_text_features(self, src_prompt=None, tgt_prompt=None):
        if getattr(self, "src_text_feature", None) is None:
            ...
            self.src_text_feature, self.src_pooled_text_feature = self.encode_text(src_prompt)
        else:
        ...
        if getattr(self, "tgt_text_feature", None) is None:
            ...
            self.tgt_text_feature, self.tgt_pooled_text_feature = self.encode_text(tgt_prompt)
        else:
    ...
    def _get_add_time_ids(self, original_size, crops_coords_top_left, target_size, dtype):
        ...
        passed_add_embed_dim = (
            self.unet.config.addition_time_embed_dim * len(add_time_ids) + 1280
        )
    ...
    def __call__(
        ...
        add_time_ids = self.add_time_ids.to(device)
        ...
        tgt_pooled_text_embedding, src_pooled_text_embedding = (
            self.tgt_pooled_text_feature,
            self.src_pooled_text_feature,
        )
        uncond_embedding = self.null_text_feature
        uncond_pooled_embedding = self.null_pooled_text_feature
        ...
        zts = dict()
        for latent, cond_text_embedding, cond_pooled_text_embedding, name in zip(
            [tgt_x0, src_x0], [tgt_text_embedding, src_text_embedding], [tgt_pooled_text_embedding, src_pooled_text_embedding], ["tgt", "src"]
        ):
            ...
            text_pooled_embeddings = torch.cat([cond_pooled_text_embedding, uncond_pooled_embedding], dim=0)
            noise_pred = self.unet.forward(
                latent_model_input,
                torch.cat([t] * 2).to(device),
                encoder_hidden_states=text_embeddings,
                added_cond_kwargs={"text_embeds": text_pooled_embeddings, "time_ids": add_time_ids},
            ).sample
    ...
    def pds_gen(
        ...
        tgt_pooled_text_embedding = self.tgt_pooled_text_feature
        uncond_embedding = self.null_text_feature
        uncond_pooled_embedding = self.null_pooled_text_feature
        ...
        text_pooled_embeddings = torch.cat([tgt_pooled_text_embedding, uncond_pooled_embedding] * batch_size, dim=0)
    ...
    def pds_gen_sdedit_src(
        ...
        src_pooled_text_embedding = self.src_pooled_text_feature
        uncond_embedding = self.null_text_feature
        uncond_pooled_embedding = self.null_pooled_text_feature
        ...
        zts = dict()
        for latent, cond_text_embedding, cond_text_pooled_embedding, name, cfg_scale in zip(
            [tgt_x0, src_x0], [tgt_text_embedding, src_text_embedding], [tgt_pooled_text_embedding, src_pooled_text_embedding], ["tgt", "src"], [tgt_cfg_scale, src_cfg_scale]
        ):
            latents_noisy = scheduler.add_noise(latent, noise, t)
            latent_model_input = torch.cat([latents_noisy] * 2, dim=0)
            text_embeddings = torch.cat([cond_text_embedding, uncond_embedding], dim=0)
            text_pooled_embeddings = torch.cat([cond_text_pooled_embedding, uncond_pooled_embedding], dim=0)
            noise_pred = self.unet.forward(
                latent_model_input,
                torch.cat([t] * 2).to(device),
                encoder_hidden_states=text_embeddings,
    ...
    def sds_loss(
        ...
        text_pooled_embeddings = torch.cat([tgt_pooled_text_embedding, uncond_pooled_embedding], dim=0)
        noise_pred = self.unet.forward(
            latent_model_input,
            torch.cat([t] * 2).to(device),
            encoder_hidden_states=text_embeddings,
    ...
    def run_sdedit(self, x0, tgt_prompt=None, num_inference_steps=20, skip=7, eta=0, add_time_ids=None):
        scheduler = self.scheduler
        ...
        null_pooled_text_feature = self.null_pooled_text_feature
        text_embeddings = torch.cat([tgt_text_embedding, null_text_embedding], dim=0)
        text_pooled_embeddings = torch.cat([tgt_pooled_text_embedding, null_pooled_text_feature], dim=0)
    ...
    def run_single_step(self, x0, t: int, tgt_prompt=None, percentile_clip=False, eta=0, add_time_ids=None):
        scheduler = self.scheduler

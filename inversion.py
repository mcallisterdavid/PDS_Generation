...
def ddim_inversion(imgname: str, num_steps: int = 50, verify: Optional[bool] = False) -> torch.Tensor:
    ...
    inv_latents, _ = pipe(prompt="A photo of an astronaut riding a horse", negative_prompt="", guidance_scale=1,
    ...
    if verify:
        ...
        # image = pipe(prompt="A photo of an astronaut riding a horse", negative_prompt="", guidance_scale=0,
        image = pipe(prompt="A photo of an cowboy riding a zebra", negative_prompt="", guidance_scale=7.5,
                     num_inference_steps=num_steps, latents=inv_latents)
        fig, ax = plt.subplots(1, 2)
        ax[0].imshow(tvt.ToPILImage()(input_img[0]))
        ax[1].imshow(image.images[0])
        plt.savefig('results/figure2/inversion_cfg1_cfg7.5.png')
    return inv_latents


if __name__ == '__main__':
    ddim_inversion('./results/figure2/A_photo_of_an_astronaut_riding_a_horse._3_7.5.jpg', num_steps=250, verify=True)